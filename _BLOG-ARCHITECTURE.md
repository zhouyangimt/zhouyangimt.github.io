...
**发布**：叙事文每天 12:10（cron `3dd0fe26383d`）——**写完直接上线，不等审核、不等图**（2026-09-09 起）

**图片**：叙事文 3 张/篇，**写作时由 agent 自己搜图并插入**（百度 acjson 接口取 ≥1200px 原图，按「整盘 / 特写 / 上桌或制作」三个角色分搜），PIL 压到 1200px≤200KB RGB JPG，按内容锚点插 `<p><img></p>`。核验闸门=**逐张全分辨率 vision**（OCR 只作粗筛，实测漏中文 overlay/角标）。占位图模式 **2026-09-15 起废除**；看门狗 cron `f6d2e9e2523f`（08:20）扫 `images/` 里的占位卡兜底，正常时完全静默。

**铁律**：叙事文禁 Amazon 链接 · 禁 emoji · 禁自动交叉链接 · 直接部署不推预览 · 配图不放宽闸门（没干净候选就重搜）
...
| # | 标题 | 格式 | 内容 | 状态 |
|---|------|------|------|------|
| **C01** | 11 件 Chinese Pantry 必备 | 产品导购 | 生抽/老抽/蚝油/料酒/香醋/豆瓣酱/花椒/五香粉/八角/淀粉/芝麻油 | ✅ 已发布 7/17 |
...
**发布**：每周 2 篇 · 周六 + 周日 10:00（cron `a72152292660`）
...
| 时间 | 板块 | 频率 |
|------|------|------|
| 08:00 | Amazon / AdSense 合规周检（周一） | 每周 1 次 |
| 08:20 | 美食补图看门狗（无待办则静默） | 每天 |
| 08:30 | 茶（周三/五） | 每周 2 篇 |
| 08:40 | SEO 周检（周一） | 每周 1 次 |
| 08:50 | Tech（周二） | 每周 1 篇 |
| 10:00 | 白酒（周六 + 周日） | 每周 2 篇 |
| 12:10 | 美食叙事（自己搜图配 3 张后直接部署） | 每天 1 篇 |
| 22:00 | 每日复盘 | 每天 |
