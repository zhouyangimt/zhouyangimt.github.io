...
这是 Hermes Agent 深度指南系列的第二篇。如果你还没看过第一篇，先去入门指南
把 Hermes 装上跑起来，本文默认你已经有了一个能用的 Hermes 实例。
...
## 配置文件在哪
Hermes 的配置分布在两个文件：`~/.hermes/config.yaml   # 主配置：模型、工具集、人格、安全策略等
~/.hermes/.env          # 密钥（API Key、Token），单独存放防止泄露
`直接编辑也行，但 Hermes 提供了更安全的方式：# 查看当前配置
hermes config
...
**第一类：聚合平台（性价比首选）**Provider特点适合场景**OpenRouter**聚合 200+ 模型，按用量计费日常主力，模型随便试**Nous Portal**Hermes 官方配套，OAuth 登录想省事的默认选择**Hugging Face**开源模型托管，免费额度跑开源模型、研究调试**第二类：厂商直连（性能最优）**Provider招牌模型适合场景**Anthropic**Claude Sonnet 4 / Opus 4复杂推理、长文分析、架构设计**OpenAI**GPT-5 / o4-mini / Codex编码、工具调用、多模态**Google Gemini**Gemini 3 Pro / Flash超长上下文（1M token）、免费额度大**DeepSeek**DeepSeek V4 Pro / Flash性价比极高，编码和推理均衡**xAI / Grok**Grok 4实时信息、创意任务**第三类：国内厂商**Provider特点**通义千问 / DashScope**阿里系，API 稳定，中文能力强**Kimi / Moonshot**月之暗面，超长上下文**GLM / Z.AI**智谱，开源生态好**MiniMax**性价比不错怎么配？两个方法：

**方法一：交互式配置**# 进入交互式模型选择器，上下箭头选
hermes model

# 或者走完整配置向导
hermes setup
`**方法二：命令行直设（适合脚本/切换）**# 切到 Claude Sonnet
hermes config set model.default &#34;anthropic/claude-sonnet-4&#34;
...
hermes config set model.default &#34;anthropic/claude-sonnet-4&#34;
`API Key 怎么设：# 直接写 .env 文件（推荐，跟 config 分离）
...
如果你一个 provider 有多个 API Key（比如多个 OpenRouter 账号，或者团队共享额度），Hermes 支持 credential pool——自动轮转 Key，一个限额了用下一个：# 多次执行 auth add，同一个 provider 会累积成池
...
### 实际场景：三模型分工
我在日常开发中用了三个模型配比：# 1. 日常主力 — DeepSeek V4 Pro
#    性价比最高，编码、问答、文档全包了
#    每天 80% 的交互用它
...
Hermes 支持 delegation——主模型把子任务分派给另一个模型：# ~/.hermes/config.yaml
delegation:
...
### 实际配置参考
这是我 ~/.hermes/config.yaml 里 model 部分的简化版：model:
...
### 常用 toolsets 速查
Toolset能力建议`terminal`执行 shell 命令、管理进程**必开**`file`读写文件、搜索、打补丁**必开**`web`网页搜索 + 内容提取**必开**`code_execution`沙箱 Python 执行写代码时开`vision`图片分析看图场景开`browser`浏览器自动化爬取动态页面时开`memory`跨会话长期记忆重度用户开`delegation`子代理任务分发复杂任务开`cronjob`定时任务管理自动化场景开`session_search`搜索历史对话项目多了开`safe`受限最低权限给别人用时开
### 怎么管 toolsets
# 交互式开关（有 TUI 菜单，最方便）
hermes tools
...
### 三个实战组合
**组合一：轻量日常（省钱、快）**hermes tools enable terminal
...
**组合二：全栈开发**hermes tools enable terminal
hermes tools enable file
hermes tools enable web
...
**组合三：自动化值班**hermes tools enable terminal
hermes tools enable file
hermes tools enable web
...
### MCP 扩展：无限工具
如果你觉得内置 toolsets 不够，Hermes 还支持 MCP 协议——接入外部工具服务器：# ~/.hermes/config.yaml
mcp_servers:
...
## 安全配置：别忽略
配置里还有几个跟安全相关的选项，顺手设了：# 密钥脱敏（默认开，别关）
...
`
## 动手试试
看到这你应该已经有一个不一样的 Hermes 了。别光看，试试这几步：- `hermes model` 切换到 DeepSeek，跑两个任务感受下- `hermes tools` 打开/关掉几个 toolset，看看 `/reset` 后工具列表变化- 试一下 delegation：把 config 里设好子代理模型，然后扔一个「帮我研究 X 主题」的任务**下一篇预告**：Skills 系统。这是 Hermes 最特别的功能——学会的东西**保存下来**，下次自动加载。相当于你给 Hermes 写 SOP，越用越顺手。我们下一篇拆解怎么写一个好用的 Skill。